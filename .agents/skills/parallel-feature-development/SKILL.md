...
description: Coordinate two or more independent feature implementations in parallel by assigning Codex subagents to isolated Git worktrees and branches, then reviewing and integrating their commits. Use when a user asks to implement multiple features, issues, or UI changes concurrently in the knoQ_R-UI repository, especially when simultaneous edits in one checkout could conflict. Do not use for a single feature, tightly coupled changes, or read-only parallel research.
---

# Parallel Feature Development

Implement independent features concurrently without allowing agents to edit the same checkout. Keep planning, integration, destructive operations, and user communication in the parent agent.

## Preconditions

1. Read the applicable `AGENTS.md` files and `docs/conventions.md` before planning.
...
5. Use available Codex subagents. If subagents are unavailable, create the worktrees only after approval and tell the user how to open one Codex chat in each worktree. Do not launch nested `codex exec` processes automatically.

## Choose The Isolation Level

Use Git worktrees by default. Read [references/isolation-strategies.md](references/isolation-strategies.md) when the user requests containers, Codespaces, cloud execution, or stronger isolation.
...
- Use Codespaces or Codex cloud only when the user asks for remote execution or local resource constraints make local work impractical. Obtain approval before creating billable or remote resources.
...
4. Identify high-conflict files such as `src/router/index.ts`, `uno.config.ts`, `package.json`, lockfiles, generated API schema, and shared UI components. Reserve them for the parent agent unless one task has exclusive ownership.
...
```bash
node .agents/skills/parallel-feature-development/scripts/worktree-manager.mjs create \
  --run 20260806-153000 \
  --base HEAD \
...
Spawn one subagent per independent implementation task, up to the available concurrency limit. Keep one parent slot available for coordination. Pass each subagent a prompt containing all of the following:
...
Read AGENTS.md and docs/conventions.md. Install dependencies with npm ci if needed.
...
If work requires a file outside the owned paths, stop that part and report the dependency to the parent agent.
...
```bash
node .agents/skills/parallel-feature-development/scripts/worktree-manager.mjs status \
  --run <run-id>
```
...
5. Ask the original agent to repair incomplete or out-of-scope work in its own worktree.
...
If integration exposes an architectural mismatch, fix it on the integration branch or send a narrowly scoped follow-up to one original agent. Do not silently broaden another agent's ownership.
...
```bash
node .agents/skills/parallel-feature-development/scripts/worktree-manager.mjs cleanup \
  --run <run-id> \
  --yes
```
