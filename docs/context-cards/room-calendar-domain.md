...
- `RoomManagementPage.vue`の一括登録は`/rooms/all`へ`text/csv`を直送し，生成スキーマ上のbody型との差をserializerと型変換で吸収している．room取得の`dateBegin`は開始時刻へ適用されるため，当日0時から取得し，終了済みroomをクライアントで除く．削除は一括APIがないため，現在ユーザーが`admins`に含まれるroomだけを選択可能にし，選択した`roomID`ごとに`DELETE /rooms/{roomID}`を呼ぶ．部分失敗したroomは選択状態で残す．これらの例外を広げず，API契約が変わった場合は呼び出し側も見直す．API変更時は[[api-data-schema]]を追加で読む．
