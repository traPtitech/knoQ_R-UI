...
| P2  | 選択が0件または削除処理中の間は削除を開始できず，確認を取り消した場合はDELETEを一度も呼ばない．                                  |
| P3  | 削除を確定すると，選択した各`roomID`にDELETEを一度ずつ実行し，成功した部屋は一覧から消え，失敗した部屋とエラー情報は画面に残る． |
...
| P2  | 未選択，選択後，確認取消，削除処理中のボタンとAPI呼び出し回数を操作試験する． | 取消時の誤削除，二重送信，0件削除．                          | DELETEが0回であるassertとボタン状態のassert．        |
| P3  | 2件選択し，DELETEを全成功と1件失敗に分けてmockする．                          | 呼び出し重複，部分失敗で成功分が残る，失敗分の選択が消える． | roomIDごとの呼び出し回数，一覧，エラー表示のassert． |
...
- **調査事項**: `GET /rooms`の期間境界が進行中の部屋を確実に含むか，削除権限が`privileged`だけでよいかは，実装中にAPI応答またはバックエンド仕様で確認する．

## Current State

- **完了**: `/rooms/manage`へCSV登録と選択削除を統合し，権限制御，旧URLのredirect，部分失敗処理，P1からP5のUIテストを実装した．Lint，型検査，本番ビルド，全12テストは成功した．
- **進行中**: Context Cardとミッション文書を更新し，Merge-Readiness Packへ検証証拠をまとめている．
- **次の一手**: 実装commitを固定し，そのSHAと最終検証結果をMerge-Readiness Packへ記録する．
