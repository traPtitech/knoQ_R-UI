...
2026-09-18のuserの「日付については絶対に現在より前と後で作りたい」という指示をP2へ追加する．初期生成時の基準日時を挟んで，通常イベント，部屋，日程調整の候補・締切に過去と未来を確保する．通常起動の基準はページ読込時の現在時刻であり，固定日時を指定した再現と空シナリオは従来どおり維持する．承認済みT-Aのデータ不変条件とHTTPテストで，月末・年末・うるう年を含む日時を検証する．
...
| P2  | 同じ依存version，シナリオ，シード，基準日時で同じ初期データを生成できる．利用者，グループ，日程調整，候補，回答，イベント，部屋の参照が解決でき，開始は終了より前になる．空シナリオを除き，イベント・部屋・日程調整候補・締切に基準日時より過去と未来の両方を含める． |
...
- **日付条件の対応完了**: 通常イベントと部屋にも過去のデータを追加した．日程調整の候補・締切を含め，初期データに過去・未来の両方があることを90件のテストと静的検査で確認した．PR #254へ反映する．
...
| 8       | 2026-09-18 | 初期データに過去と未来を必ず含める条件をP2へ追加した．                                  | userの「日付については絶対に現在より前と後で作りたい」に基づく．既存のデータ不変条件・HTTPテストで検証する．   |
